  ...
  return (
    <Fragment>
      ...
      <Form layout='horizontal' requiredMarkStyle='asterisk' footer={
        ...
        <Form.Item label='招聘职位' rules={[{ required: true, message: '招聘职位必须指定' }]}>
          <Input onChange={console.log} placeholder='请输入招聘职位' />
        </Form.Item>
        <Form.Item label='公司名称' rules={[{ required: true, message: '公司名称必须指定' }]}>
          <Input onChange={console.log} placeholder='请输入公司名称' />
        </Form.Item>
        <Form.Item label='职位薪资' rules={[{ required: true, message: '职位薪资必须指定' }]}>
...
export default connect()(ApplicantInfo)
